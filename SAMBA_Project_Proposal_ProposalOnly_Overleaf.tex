...
The following screenshots are preliminary dashboard views from the local React/Material UI interface. They are included to show how the final results will be presented. The current dashboard is connected to simulator output files, but the final report will update these views after the complete experiment matrix is finished.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{docs/figures/dashboard_headline.png}
\caption{Preliminary React/Material UI dashboard headline view showing SAMBA speedup and energy-efficiency summaries.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{docs/figures/dashboard_variants.png}
\caption{Preliminary variant comparison view showing fixed ADC, sparse ADC, ablation variants, and SAMBA-style results.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{docs/figures/dashboard_layers.png}
\caption{Preliminary layer-level bottleneck view showing latency, movement cycles, utilization, and load-balancing counts.}
\end{figure}
...
The proposal includes preliminary source code as required by the assignment. The repository link can be added here before submission:
...
\begin{itemize}
    ...
    \item \texttt{dashboard/}: Planned React/Material UI dashboard.
